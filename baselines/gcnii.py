#!/usr/bin/env python3
"""Standalone, model-only GCNII baseline runner.

Example: python gcnii.py 0
Dataset indices: 0=Cora, 1=Citeseer, 2=Pubmed.
Only the GCNII implementation and the shared data/training path needed by it are included.
"""
...
class GCNIIModel(nn.Module):
    ...
    def __init__(self, graph: GraphData, hidden: int=64, classes: Optional[int]=None, layers: int=64, dropout: float=0.6, alpha: float=0.1, lam: float=0.5):
        super().__init__()
        classes = graph.num_classes if classes is None else classes
        self.dropout = float(dropout)
        self.alpha = float(alpha)
        self.lam = float(lam)
        self.lin_in = nn.Linear(graph.num_features, hidden)
        self.convs = nn.ModuleList([GCNIIConv(hidden) for _ in range(int(layers))])
        self.lin_out = nn.Linear(hidden, classes)

    def forward(self, graph: GraphData) -> Tensor:
        edge_index, edge_weight = gcn_norm(graph.edge_index, graph.num_nodes, graph.x.dtype, add_loops=False)
        ...
        for idx, conv in enumerate(self.convs, start=1):
            ...
            h = F.relu(conv(h, h0, edge_index, edge_weight, self.alpha, self.lam, idx))
        h = F.dropout(h, p=self.dropout, training=self.training)
...
def model_config(
    ...
    cfg = TrainConfig(hidden=64, layers=64, dropout=0.6, extras={'alpha': 0.1, 'lam': 0.5})
    if quick:
...
def build_model(graph: GraphData, config: TrainConfig) -> nn.Module:
    ...
    return GCNIIModel(graph, config.hidden, graph.num_classes, config.layers, config.dropout, alpha=float(extras['alpha']), lam=float(extras['lam']))
