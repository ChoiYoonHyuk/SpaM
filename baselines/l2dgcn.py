#!/usr/bin/env python3
"""Standalone, model-only L2DGCN baseline runner.

Example: python l2dgcn.py 0
Dataset indices: 0=Cora, 1=Citeseer, 2=Pubmed.
Only the L2DGCN implementation and the shared data/training path needed by it are included.
"""
...
try:
    ...
    raise RuntimeError('scikit-learn is required for this baseline: pip install scikit-learn') from exc
...
class L2DGCNModel(nn.Module):

    def __init__(self, graph: GraphData, hidden: int=64, steps: int=10, alpha: float=0.1, beta: float=0.1, beta_w: float=0.2, knn_p: int=10, mu: float=0.9, pseudo_weight: float=1.0, contrast_weight: float=0.1, temperature: float=0.5, refresh_interval: int=10):
        super().__init__()
        self.w1 = nn.Linear(graph.num_features, hidden)
        self.w2 = nn.Linear(hidden, graph.num_classes)
        self.steps = int(steps)
        self.alpha = float(alpha)
        self.beta = float(beta)
        self.beta_w = float(beta_w)
        self.knn_p = int(knn_p)
        self.mu = float(mu)
        self.pseudo_weight = float(pseudo_weight)
        self.contrast_weight = float(contrast_weight)
        self.temperature = float(temperature)
        self.refresh_interval = int(refresh_interval)
        self.register_buffer('enhanced_edge', graph.edge_index.detach().clone())
        self.register_buffer('enhanced_weight', row_norm(graph.edge_index, graph.num_nodes, graph.x.dtype, by='dst'))
        self.teacher_probs: Optional[Tensor] = None
        self.last_hidden: Optional[Tensor] = None
        self.last_student_probs: Optional[Tensor] = None

    @torch.no_grad()
    def _enhance_graph(self, graph: GraphData, representation: Tensor) -> None:
        base = remove_self_loops(graph.edge_index).detach().cpu()
        src, dst = base
        deg = torch.zeros(graph.num_nodes)
        deg.index_add_(0, src, torch.ones(src.numel()))
        m = max(1, min(graph.num_nodes, int(round((1.0 - self.beta_w) * graph.num_nodes))))
        selected_idx = torch.topk(deg, k=m, largest=True).indices
        selected = torch.zeros(graph.num_nodes, dtype=torch.bool)
        selected[selected_idx] = True
        keep = selected[src] | selected[dst]
        kept_edge = base[:, keep]
        unselected = torch.nonzero(~selected, as_tuple=False).view(-1).numpy()
        added: List[np.ndarray] = []
        if unselected.size > 1 and self.knn_p > 0:
            rep = representation.detach().cpu().float().numpy()[unselected]
            neighbors = min(self.knn_p + 1, unselected.size)
            nn_index = NearestNeighbors(n_neighbors=neighbors, metric='cosine', algorithm='brute')
            nn_index.fit(rep)
            _, indices = nn_index.kneighbors(rep)
            for row_i, local_neighbors in enumerate(indices):
                i = int(unselected[row_i])
                for local_j in local_neighbors[1:]:
                    j = int(unselected[int(local_j)])
                    added.append(np.array([[i, j], [j, i]], dtype=np.int64).T)
        if added:
            new_edges = torch.from_numpy(np.concatenate(added, axis=1)).long()
            enhanced = torch.cat([kept_edge, new_edges], dim=1)
        else:
            enhanced = kept_edge
        enhanced = add_remaining_self_loops(to_undirected_edges(enhanced, graph.num_nodes), graph.num_nodes)
        self.enhanced_edge = enhanced.to(graph.x.device)
        self.enhanced_weight = row_norm(self.enhanced_edge, graph.num_nodes, graph.x.dtype, by='dst')

    @torch.no_grad()
    def _teacher(self, graph: GraphData) -> Tensor:
        y0 = torch.zeros(graph.num_nodes, graph.num_classes, device=graph.x.device, dtype=graph.x.dtype)
        y0[graph.train_mask] = one_hot(graph.y[graph.train_mask], graph.num_classes, graph.x.dtype)
        h = y0
        for _ in range(self.steps):
            h = (1.0 - self.alpha) * propagate(h, self.enhanced_edge, self.enhanced_weight) + self.alpha * y0
        return normalize_probs(h)

    @torch.no_grad()
    def on_epoch_start(self, graph: GraphData, epoch: int) -> None:
        if epoch == 1 or (self.refresh_interval > 0 and epoch % self.refresh_interval == 0):
            representation = self.last_hidden if self.last_hidden is not None else graph.x
            self._enhance_graph(graph, representation)
            self.teacher_probs = self._teacher(graph)

    def _propagate_restart(self, x: Tensor) -> Tensor:
        h0 = x
        h = x
        for _ in range(self.steps):
            h = (1.0 - self.beta) * propagate(h, self.enhanced_edge, self.enhanced_weight) + self.beta * h0
        return h

    def forward(self, graph: GraphData) -> Tensor:
        x_prop = self._propagate_restart(graph.x)
        hidden = F.relu(self.w1(x_prop))
        logits = self.w2(hidden)
        y0 = self.w2(F.relu(self.w1(graph.x)))
        y_prop = self._propagate_restart(y0)
        self.last_hidden = hidden
        self.last_student_probs = torch.softmax(y_prop, dim=-1)
        return logits

    def extra_loss(self, graph: GraphData, logits: Tensor, epoch: int) -> Tensor:
        del epoch
        if self.teacher_probs is None or self.last_student_probs is None or self.last_hidden is None:
            return logits.new_tensor(0.0)
        student_conf, student_pred = self.last_student_probs.max(dim=-1)
        teacher_pred = self.teacher_probs.argmax(dim=-1)
        pseudo_mask = ~graph.train_mask & (student_conf >= self.mu)
        extra = logits.new_tensor(0.0)
        if pseudo_mask.any():
            extra = extra + self.pseudo_weight * F.cross_entropy(logits[pseudo_mask], teacher_pred[pseudo_mask])
        reliable = graph.train_mask | pseudo_mask
        if reliable.sum().item() > 1:
            labels = graph.y.clone()
            labels[pseudo_mask] = teacher_pred[pseudo_mask]
            z = F.normalize(self.last_hidden[reliable], p=2, dim=-1)
            rel_y = labels[reliable]
            prototypes = []
            valid_classes = []
            for c in range(graph.num_classes):
                mask = rel_y == c
                if mask.any():
                    prototypes.append(F.normalize(z[mask].mean(dim=0, keepdim=True), p=2, dim=-1).squeeze(0))
                    valid_classes.append(c)
            if len(prototypes) >= 2:
                proto = torch.stack(prototypes)
                target_map = {c: i for i, c in enumerate(valid_classes)}
                target = torch.tensor([target_map[int(v)] for v in rel_y.tolist()], device=z.device)
                contrast_logits = z @ proto.t() / max(self.temperature, 1e-06)
                extra = extra + self.contrast_weight * F.cross_entropy(contrast_logits, target)
        return extra
...
def model_config(
    ...
    cfg = TrainConfig(epochs=100, patience=20, lr=0.01, weight_decay=5e-05, hidden=64, layers=10, dropout=0.0, extras={'alpha': 0.1, 'beta': 0.1, 'beta_w': 0.2, 'knn_p': 10, 'mu': 0.9, 'pseudo_weight': 1.0, 'contrast_weight': 0.1, 'temperature': 0.5, 'refresh_interval': 1})
    if quick:
...
def build_model(graph: GraphData, config: TrainConfig) -> nn.Module:
    ...
    return L2DGCNModel(graph, config.hidden, config.layers, float(extras['alpha']), float(extras['beta']), float(extras['beta_w']), int(extras['knn_p']), float(extras['mu']), float(extras['pseudo_weight']), float(extras['contrast_weight']), float(extras['temperature']), int(extras['refresh_interval']))
...
def train_model_instance(
    ...
    for epoch in range(1, config.epochs + 1):
        model.on_epoch_start(graph, epoch)
        model.train()
        optimizer.zero_grad(set_to_none=True)
        logits = model(graph)
        loss = F.cross_entropy(logits[graph.train_mask], graph.y[graph.train_mask])
        loss = loss + model.extra_loss(graph, logits, epoch)
