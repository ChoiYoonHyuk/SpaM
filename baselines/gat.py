#!/usr/bin/env python3
"""Standalone, model-only GAT baseline runner.

Example: python gat.py 0
Dataset indices: 0=Cora, 1=Citeseer, 2=Pubmed.
Only the GAT implementation and the shared data/training path needed by it are included.
"""
...
class SparseGATLayer(nn.Module):
    ...
    def forward(self, x: Tensor, edge_index: Tensor) -> Tensor:
        num_nodes = x.size(0)
        z = self.linear(x).view(num_nodes, self.heads, self.head_dim)
        src, dst = edge_index
        ...
        messages = z[src] * alpha.unsqueeze(-1)
        out = z.new_zeros((num_nodes, self.heads, self.head_dim))
        out.index_add_(0, dst, messages)
        return out.reshape(num_nodes, self.heads * self.head_dim)
...
class GATModel(nn.Module):
    def __init__(
        ...
        self.input_linear = (
            nn.Linear(graph.num_features, hidden) if self.pre_linear else None
        )
        self.convs = nn.ModuleList()
        self.res_lins = nn.ModuleList() if self.residual else None
        self.norms = nn.ModuleList() if norm != 'none' else None
        in_dim = hidden if self.pre_linear else graph.num_features
        for _ in range(int(local_layers)):
            self.convs.append(
                SparseGATLayer(in_dim, hidden, heads=heads, dropout=dropout)
            )
            if self.res_lins is not None:
                self.res_lins.append(nn.Linear(in_dim, hidden))
            if self.norms is not None:
                self.norms.append(
                    nn.BatchNorm1d(hidden) if norm == 'bn' else nn.LayerNorm(hidden)
                )
            in_dim = hidden
    ...
    def forward(self, graph: GraphData) -> Tensor:
        ...
        for index, conv in enumerate(self.convs):
            h = conv(x, edge_index)
            if self.res_lins is not None:
                h = h + self.res_lins[index](x)
            if self.norms is not None:
                h = self.norms[index](h)
            x = F.dropout(F.relu(h), p=self.dropout, training=self.training)
...
def model_config(
    ...
    if data_id in PLANETOID_NAMES:
        item = dict(epochs=1000, patience=100, lr=0.005, weight_decay=0.0005,
                    hidden=64, layers=2, dropout=0.6, heads=8,
                    pre_linear=False, residual=False, norm='none')
    else:
        table = {
            3: dict(epochs=1000, patience=250, lr=0.01, weight_decay=0.0005,
                    hidden=256, layers=2, dropout=0.7, heads=1,
                    pre_linear=False, residual=True, norm='bn'),
            4: dict(epochs=1000, patience=250, lr=0.005, weight_decay=0.0005,
                    hidden=512, layers=7, dropout=0.5, heads=1,
                    pre_linear=False, residual=True, norm='bn'),
        }
        item = table.get(data_id, dict(epochs=1000, patience=200, lr=0.01,
                    weight_decay=0.0005, hidden=64, layers=2, dropout=0.5,
                    heads=8, pre_linear=False, residual=False, norm='none'))
    cfg = TrainConfig(
        epochs=int(item['epochs']), patience=int(item['patience']),
        lr=float(item['lr']), weight_decay=float(item['weight_decay']),
        hidden=int(item['hidden']), layers=int(item['layers']),
        dropout=float(item['dropout']),
        extras={'heads': int(item['heads']), 'pre_linear': bool(item['pre_linear']),
                'residual': bool(item['residual']), 'norm': str(item['norm'])},
    )
    if quick:
        cfg.epochs = min(cfg.epochs, 25)
        cfg.patience = min(cfg.patience, 8)
        cfg.hidden = min(cfg.hidden, 64)
        cfg.layers = min(cfg.layers, 4)
...
def build_model(graph: GraphData, config: TrainConfig) -> nn.Module:
    ...
    return GATModel(
        ...
        heads=int(extras['heads']),
        pre_linear=bool(extras['pre_linear']),
        residual=bool(extras['residual']),
        norm=str(extras['norm']),
    )
