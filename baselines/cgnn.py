#!/usr/bin/env python3
"""Standalone, model-only CGNN baseline runner.

Example: python cgnn.py 0
Dataset indices: 0=Cora, 1=Citeseer, 2=Pubmed.
Only the CGNN implementation and the shared data/training path needed by it are included.
"""
...
def _stationary_distribution(p: sp.csr_matrix, max_iter: int=10000, tol: float=1e-13) -> np.ndarray:
    n = p.shape[0]
    pi = np.full(n, 1.0 / n, dtype=np.float64)
    for _ in range(max_iter):
        new = np.asarray(p.T @ pi).reshape(-1)
...
def cgnn_proximity(graph: GraphData, rank: int=5, random_state: int=0) -> Tuple[Tensor, Tensor, Tensor]:
    x = graph.x.detach().cpu().numpy().astype(np.float64, copy=False)
    edge_original = graph.edge_index.detach().cpu()
    n = graph.num_nodes
    anchor = x.mean(axis=0)
    denom = np.linalg.norm(x, axis=1) * max(np.linalg.norm(anchor), 1e-12)
    similarity = x @ anchor / np.maximum(denom, 1e-12)
    order = np.argsort(similarity)
    chain_src = np.concatenate([order[:-1], order[1:]])
    chain_dst = np.concatenate([order[1:], order[:-1]])
    loops = np.arange(n)
    extra = torch.tensor(np.vstack([np.concatenate([chain_src, loops]), np.concatenate([chain_dst, loops])]), dtype=torch.long)
    rewired = coalesce_edge_index(torch.cat([edge_original, extra], dim=1), n)
    src = rewired[0].numpy()
    dst = rewired[1].numpy()
    outdeg = np.bincount(src, minlength=n).astype(np.float64)
    transition_weight = 1.0 / np.maximum(outdeg[src], 1.0)
    p = sp.coo_matrix((transition_weight, (src, dst)), shape=(n, n)).tocsr()
    pi = _stationary_distribution(p)
    rows = np.concatenate([src, dst, src, dst])
    cols = np.concatenate([src, dst, dst, src])
    vals = np.concatenate([transition_weight, transition_weight, -transition_weight, -transition_weight])
    t = sp.coo_matrix((vals, (rows, cols)), shape=(n, n)).tocsr()
    weighted_t = sp.diags(pi) @ t
    q = max(1, min(int(rank), n - 1))
    try:
        u, s, vt = randomized_svd(weighted_t, n_components=q, n_iter=7, random_state=random_state)
    except Exception:
        dense = weighted_t.toarray()
        u0, s0, vt0 = np.linalg.svd(dense, full_matrices=False)
        u, s, vt = (u0[:, :q], s0[:q], vt0[:q])
    inv_s = 1.0 / np.maximum(s, 1e-12)
    left = u * inv_s.reshape(1, -1)
    right = vt.T
    diag = np.sum(left * right, axis=1)
    e = edge_original.numpy()
    u_idx, v_idx = (e[0], e[1])
    t_uv = np.sum(left[u_idx] * right[v_idx], axis=1)
    t_vu = np.sum(left[v_idx] * right[u_idx], axis=1)
    sqrt_pi = np.sqrt(np.maximum(pi[u_idx] * pi[v_idx], 1e-300))
    commute = diag[v_idx] / np.maximum(pi[v_idx], 1e-300) + diag[u_idx] / np.maximum(pi[u_idx], 1e-300)
    commute -= (t_uv + t_vu) / sqrt_pi
    commute = np.nan_to_num(commute, nan=50.0, posinf=50.0, neginf=0.0)
    commute = np.clip(commute, 0.0, 50.0)
    prox = np.exp(-commute)
    src_t, dst_t = edge_original
    prox_t = torch.tensor(prox, dtype=torch.float32)
    out_max = torch.zeros(n, dtype=torch.float32)
    in_max = torch.zeros(n, dtype=torch.float32)
    out_max.scatter_reduce_(0, src_t, prox_t, reduce='amax', include_self=True)
    in_max.scatter_reduce_(0, dst_t, prox_t, reduce='amax', include_self=True)
    out_weight = prox_t / out_max[src_t].clamp_min(1e-12)
    in_weight = prox_t / in_max[dst_t].clamp_min(1e-12)
    return (edge_original, in_weight, out_weight)
...
class CGNNLayer(nn.Module):
    ...
    def __init__(self, hidden: int):
        super().__init__()
        self.in_transform = nn.Linear(hidden, hidden, bias=False)
        self.out_transform = nn.Linear(hidden, hidden, bias=False)

    def forward(self, h: Tensor, edge_index: Tensor, in_weight: Tensor, out_weight: Tensor) -> Tensor:
        src, dst = edge_index
        in_msg = self.in_transform(h)[src] * in_weight.to(h.dtype).unsqueeze(-1)
        m_in = h.new_zeros(h.shape)
        m_in.index_add_(0, dst, in_msg)
        in_deg = h.new_zeros(h.size(0))
        in_deg.index_add_(0, dst, torch.ones(dst.numel(), dtype=h.dtype, device=h.device))
        m_in = m_in / in_deg.clamp_min(1.0).unsqueeze(-1)
        out_msg = self.out_transform(h)[dst] * out_weight.to(h.dtype).unsqueeze(-1)
        m_out = h.new_zeros(h.shape)
        m_out.index_add_(0, src, out_msg)
        out_deg = h.new_zeros(h.size(0))
        out_deg.index_add_(0, src, torch.ones(src.numel(), dtype=h.dtype, device=h.device))
        m_out = m_out / out_deg.clamp_min(1.0).unsqueeze(-1)
        return (h + m_in + m_out) / 3.0


class CGNNModel(nn.Module):

    def __init__(self, graph: GraphData, hidden: int=64, layers: int=2, dropout: float=0.5, rank: int=5, seed: int=0):
        super().__init__()
        edge, in_w, out_w = cgnn_proximity(graph, rank=rank, random_state=seed)
        self.register_buffer('commute_edge', edge)
        self.register_buffer('commute_in_weight', in_w)
        self.register_buffer('commute_out_weight', out_w)
        self.encoder = nn.Linear(graph.num_features, hidden)
        self.layers = nn.ModuleList([CGNNLayer(hidden) for _ in range(int(layers))])
        self.mlp = nn.Sequential(nn.Linear(hidden, hidden), nn.ReLU(), nn.Dropout(dropout), nn.Linear(hidden, graph.num_classes))
        self.dropout = float(dropout)

    def forward(self, graph: GraphData) -> Tensor:
        h = F.relu(self.encoder(graph.x))
        for layer in self.layers:
            h = F.relu(layer(h, self.commute_edge, self.commute_in_weight, self.commute_out_weight))
...
def model_config(
    ...
    if data_id == 3:
        cfg = TrainConfig(weight_decay=0.0, hidden=128, layers=4, dropout=0.5)
    elif data_id == 4:
        cfg = TrainConfig(lr=0.005, weight_decay=0.0, hidden=128, layers=5, dropout=0.5)
    else:
...
def build_model(graph: GraphData, config: TrainConfig, seed: int) -> nn.Module:
    return CGNNModel(
        graph, config.hidden, config.layers, config.dropout,
        int(config.extras['rank']), seed,
    )
...
def train_one(
    ...
    set_seed(seed)
    model = build_model(graph, config, seed)
