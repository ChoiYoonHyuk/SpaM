#!/usr/bin/env python3
"""Standalone, model-only GBK-GNN baseline runner.

Example: python gbk_gnn.py 0
Dataset indices: 0=Cora, 1=Citeseer, 2=Pubmed.
Only the GBK-GNN implementation and the shared data/training path needed by it are included.
"""
...
class GBKConv(nn.Module):

    def __init__(self, in_dim: int, out_dim: int, negative_slope: float=0.2):
        super().__init__()
        self.gate_linear = nn.Linear(in_dim, out_dim, bias=False)
        self.att_l = nn.Parameter(torch.empty(1, out_dim))
        self.att_r = nn.Parameter(torch.empty(1, out_dim))
        self.weight_p = nn.Parameter(torch.empty(in_dim, out_dim))
        self.weight_n = nn.Parameter(torch.empty(in_dim, out_dim))
        self.bias = nn.Parameter(torch.zeros(out_dim))
        self.negative_slope = float(negative_slope)
        self.last_sigma: Optional[Tensor] = None
        self.reset_parameters()

    def reset_parameters(self) -> None:
        for p in (self.gate_linear.weight, self.att_l, self.att_r, self.weight_p, self.weight_n):
            nn.init.xavier_uniform_(p)
        nn.init.zeros_(self.bias)

    def forward(self, x: Tensor, edge_index: Tensor, edge_weight: Tensor) -> Tensor:
        z = self.gate_linear(x)
        sigma_l = (z * self.att_l).sum(dim=-1)
        sigma_r = (z * self.att_r).sum(dim=-1)
        x_p, x_n = (x @ self.weight_p, x @ self.weight_n)
        src, dst = edge_index
        sigma = torch.sigmoid(F.leaky_relu(sigma_l[src] + sigma_r[dst], self.negative_slope))
        self.last_sigma = sigma
        msg = edge_weight.unsqueeze(-1) * (x_p[src] * sigma.unsqueeze(-1) + x_n[dst] * (1.0 - sigma).unsqueeze(-1))
        out = x_p.new_zeros((x.size(0), x_p.size(1)))
        out.index_add_(0, dst, msg)
        return out + self.bias


class GBKGNNModel(nn.Module):

    def __init__(self, graph: GraphData, hidden: int=16, dropout: float=0.5, lamda: float=1.84):
        super().__init__()
        self.conv1 = GBKConv(graph.num_features, hidden)
        self.conv2 = GBKConv(hidden, graph.num_classes)
        self.dropout = float(dropout)
        self.lamda = float(lamda)

    def forward(self, graph: GraphData) -> Tensor:
        edge, weight = gcn_norm(graph.edge_index, graph.num_nodes, graph.x.dtype, add_loops=False)
        x = F.relu(self.conv1(graph.x, edge, weight))
        x = F.dropout(x, p=self.dropout, training=self.training)
        return self.conv2(x, edge, weight)

    def extra_loss(self, graph: GraphData, logits: Tensor, epoch: int) -> Tensor:
        del logits, epoch
        src, dst = graph.edge_index
        train_edge = graph.train_mask[src] & graph.train_mask[dst]
        if not train_edge.any():
            return graph.x.new_tensor(0.0)
        target = (graph.y[src] == graph.y[dst]).long()[train_edge]
        losses = []
        for conv in (self.conv1, self.conv2):
            if conv.last_sigma is None:
                continue
            sigma = conv.last_sigma[train_edge].clamp(1e-07, 1.0 - 1e-07)
            two_class = torch.stack([1.0 - sigma, sigma], dim=1)
            losses.append(F.nll_loss(two_class.log(), target))
        return self.lamda * torch.stack(losses).sum() if losses else graph.x.new_tensor(0.0)
...
def model_config(
    ...
    cfg = TrainConfig(epochs=500, lr=0.001, weight_decay=0.01, hidden=16, layers=2, dropout=0.5, extras={'lamda': 1.84})
    if quick:
...
def build_model(graph: GraphData, config: TrainConfig) -> nn.Module:
    ...
    return GBKGNNModel(graph, config.hidden, config.dropout, float(extras['lamda']))
