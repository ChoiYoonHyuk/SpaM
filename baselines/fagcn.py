#!/usr/bin/env python3
"""Standalone, model-only FAGCN baseline runner.

Example: python fagcn.py 0
Dataset indices: 0=Cora, 1=Citeseer, 2=Pubmed.
Only the FAGCN implementation and the shared data/training path needed by it are included.
"""
...
class FAGCNModel(nn.Module):
    ...
    def __init__(self, graph: GraphData, hidden: int=64, dropout: float=0.5, eps: float=0.1, layers: int=2):
        super().__init__()
        self.t1 = nn.Linear(graph.num_features, hidden)
        self.t2 = nn.Linear(hidden, graph.num_classes)
        self.layers = nn.ModuleList([FALayer(hidden, dropout) for _ in range(int(layers))])
    ...
    def forward(self, graph: GraphData) -> Tensor:
        h = F.dropout(graph.x, p=self.dropout, training=self.training)
        h = F.relu(self.t1(h))
        h = F.dropout(h, p=self.dropout, training=self.training)
        raw = h
        for layer in self.layers:
            h = layer(h, graph.edge_index, graph.num_nodes)
            h = self.eps * raw + h
        return self.t2(h)
...
def model_config(
    ...
    cfg = TrainConfig(hidden=64, layers=2, dropout=0.5, extras={'eps': 0.1})
    if quick:
...
def build_model(graph: GraphData, config: TrainConfig) -> nn.Module:
    ...
    return FAGCNModel(graph, config.hidden, config.dropout, float(extras['eps']), config.layers)
