#!/usr/bin/env python3
"""Standalone, model-only TFE-GNN baseline runner.

Example: python tfe_gnn.py 0
Dataset indices: 0=Cora, 1=Citeseer, 2=Pubmed.
Only the TFE-GNN implementation and the shared data/training path needed by it are included.
"""
...
class TFEGNNModel(nn.Module):

    def __init__(self, graph: GraphData, hidden: int=64, dropout: float=0.5):
        super().__init__()
        self.encoder = nn.Linear(graph.num_features, hidden)
        self.channel_maps = nn.ModuleList([nn.Linear(hidden, hidden) for _ in range(3)])
        self.gate = nn.Sequential(nn.Linear(hidden * 3, hidden), nn.ReLU(), nn.Linear(hidden, 3))
    ...
    def forward(self, graph: GraphData) -> Tensor:
        ...
        z = F.relu(self.encoder(F.dropout(graph.x, p=self.dropout, training=self.training)))
        low1 = propagate(z, edge, weight)
        low2 = propagate(low1, edge, weight)
        channels = [z, 0.5 * (low1 + low2), z - low1]
        transformed = [F.relu(layer(h)) for layer, h in zip(self.channel_maps, channels)]
        gate = torch.softmax(self.gate(torch.cat(transformed, dim=-1)), dim=-1)
        fused = sum((gate[:, i:i + 1] * transformed[i] for i in range(3)))
...
def build_model(graph: GraphData, config: TrainConfig) -> nn.Module:
    ...
    return TFEGNNModel(graph, config.hidden, config.dropout)
