#!/usr/bin/env python3
"""Standalone, model-only GloGNN baseline runner.

Example: python glognn.py 0
Dataset indices: 0=Cora, 1=Citeseer, 2=Pubmed.
Only the GloGNN implementation and the shared data/training path needed by it are included.
"""
...
class GloGNNModel(nn.Module):

    def __init__(self, graph: GraphData, hidden: int=16, dropout: float=0.5, alpha: float=0.1, beta: float=0.1, gamma: float=0.2, delta: float=1.0, norm_layers: int=2, orders: int=2):
        super().__init__()
        self.fc1 = nn.Linear(graph.num_features, hidden)
        self.fc2 = nn.Linear(hidden, graph.num_classes)
        self.fc3 = nn.Linear(graph.num_nodes, hidden)
        self.dropout = float(dropout)
        self.alpha = float(alpha)
        self.beta = float(beta)
        self.gamma = float(gamma)
        self.delta = float(delta)
        self.norm_layers = int(norm_layers)
        self.orders = int(orders)
        self.orders_weight = nn.Parameter(torch.ones(orders, 1) / max(1, orders))
        self.orders_weight_matrix = nn.Parameter(torch.empty(graph.num_classes, orders))
        self.orders_weight_matrix2 = nn.Parameter(torch.empty(orders, orders))
        self.diag_weight = nn.Parameter(torch.ones(graph.num_classes, 1) / graph.num_classes)
    ...
    def _order_func3(self, x: Tensor, res: Tensor, edge_index: Tensor, edge_weight: Tensor) -> Tensor:
        orders_para = torch.relu(x @ self.orders_weight_matrix) @ self.orders_weight_matrix2
        orders_para = orders_para.t()
        tmp = propagate(res, edge_index, edge_weight)
        total = orders_para[0].unsqueeze(1) * tmp
        for i in range(1, self.orders):
            tmp = propagate(tmp, edge_index, edge_weight)
            total = total + orders_para[i].unsqueeze(1) * tmp
        return total

    def _norm_func2(self, x: Tensor, h0: Tensor, edge_index: Tensor, edge_weight: Tensor) -> Tensor:
        coe = 1.0 / (self.alpha + self.beta)
        coe1 = 1.0 - self.gamma
        coe2 = 1.0 / coe1
        gram = x.t() @ x
        eye = torch.eye(x.size(1), device=x.device, dtype=x.dtype)
        inv = torch.linalg.inv(coe2 * coe2 * eye + coe * gram)
        res = inv @ gram
        res = (coe1 * coe * x - coe1 * coe * coe * (x @ res)) * self.diag_weight.t().to(x.dtype)
        tmp = self.diag_weight.to(x.dtype) * (x.t() @ res)
        sum_orders = self._order_func3(x, res, edge_index, edge_weight)
        return coe1 * (x @ tmp) + self.beta * sum_orders - self.gamma * coe1 * (h0 @ tmp) + self.gamma * h0

    def forward(self, graph: GraphData) -> Tensor:
        dtype = self.fc1.weight.dtype
        x = row_normalize_features(graph.x).to(dtype)
        x_x = self.fc1(x)
        if self.delta != 1.0:
            adj_dense = x.new_zeros((graph.num_nodes, graph.num_nodes))
            adj_dense[graph.edge_index[0], graph.edge_index[1]] = 1.0
            x_a = self.fc3(adj_dense)
            h = F.relu(self.delta * x_x + (1.0 - self.delta) * x_a)
        else:
            h = F.relu(x_x)
        h = F.dropout(h, p=self.dropout, training=self.training)
        h = self.fc2(h)
        h0 = h
        edge, weight = gcn_norm(graph.edge_index, graph.num_nodes, h.dtype, add_loops=False)
        weight = row_norm(edge, graph.num_nodes, h.dtype, by='dst')
        for _ in range(self.norm_layers):
            h = self._norm_func2(h, h0, edge, weight)
        return h
...
def model_config(
    ...
    cfg = TrainConfig(patience=40, hidden=16, layers=2, dropout=0.5, extras={'alpha': 0.1, 'beta': 0.1, 'gamma': 0.2, 'delta': 1.0, 'orders': 2})
    if quick:
...
def build_model(graph: GraphData, config: TrainConfig) -> nn.Module:
    ...
    return GloGNNModel(graph, config.hidden, config.dropout, float(extras['alpha']), float(extras['beta']), float(extras['gamma']), float(extras['delta']), config.layers, int(extras['orders'])).double()
