#!/usr/bin/env python3
"""Standalone, model-only GCN baseline runner.

Example: python gcn.py 0
Dataset indices: 0=Cora, 1=Citeseer, 2=Pubmed.
Only the GCN implementation and the shared data/training path needed by it are included.
"""
...
class GCNModel(nn.Module):
    def __init__(
        ...
        for _ in range(int(local_layers)):
            self.convs.append(SparseGCNLayer(in_dim, hidden, bias=True))
    ...
    def forward(self, graph: GraphData) -> Tensor:
        ...
        edge_index, edge_weight = gcn_norm(
            graph.edge_index, graph.num_nodes, x.dtype, add_loops=False
        )
...
def model_config(
    ...
    if data_id in PLANETOID_NAMES:
        item = dict(epochs=1000, patience=100, lr=0.01, weight_decay=0.0005,
                    hidden=64, layers=2, dropout=0.5, pre_linear=False,
                    residual=False, norm='none')
    else:
        table = {
            3: dict(epochs=1000, patience=250, lr=0.005, weight_decay=0.001,
                    hidden=512, layers=5, dropout=0.2, pre_linear=False,
                    residual=True, norm='bn'),
            4: dict(epochs=1000, patience=250, lr=0.01, weight_decay=0.0005,
                    hidden=256, layers=4, dropout=0.7, pre_linear=False,
                    residual=True, norm='bn'),
        }
    ...
    cfg = TrainConfig(
        ...
        extras={'pre_linear': bool(item['pre_linear']),
