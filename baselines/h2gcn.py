#!/usr/bin/env python3
"""Standalone, model-only H2GCN baseline runner.

Example: python h2gcn.py 0
Dataset indices: 0=Cora, 1=Citeseer, 2=Pubmed.
Only the H2GCN implementation and the shared data/training path needed by it are included.
"""
...
class H2GCNModel(nn.Module):
    ...
    def __init__(self, graph: GraphData, hidden: int=64, layers: int=2, dropout: float=0.5):
        super().__init__()
        self.hidden = int(hidden)
        self.depth = int(layers)
        self.dropout = float(dropout)
        hops = strict_hop_edges(graph, 2)
        self.register_buffer('hop1_edge', hops[0][0])
        self.register_buffer('hop1_weight', hops[0][1])
        self.register_buffer('hop2_edge', hops[1][0])
        self.register_buffer('hop2_weight', hops[1][1])
        self.embed = nn.Linear(graph.num_features, hidden)
        final_dim = hidden * (2 ** (self.depth + 1) - 1)
        self.classifier = nn.Linear(final_dim, graph.num_classes)
    ...
    def forward(self, graph: GraphData) -> Tensor:
        ...
        for _ in range(self.depth):
            h1 = propagate(current, self.hop1_edge, self.hop1_weight)
            h2 = propagate(current, self.hop2_edge, self.hop2_weight)
            current = torch.cat([h1, h2], dim=-1)
...
def build_model(graph: GraphData, config: TrainConfig) -> nn.Module:
    ...
    return H2GCNModel(graph, config.hidden, config.layers, config.dropout)
