#!/usr/bin/env python3
"""Standalone, model-only Geom-GCN baseline runner.

Example: python geom_gcn.py 0
Dataset indices: 0=Cora, 1=Citeseer, 2=Pubmed.
Only the Geom-GCN implementation and the shared data/training path needed by it are included.
"""
...
def build_geom_relations(graph: GraphData, bins: int=8, random_state: int=0) -> Tuple[Tensor, Tensor]:
    edge = remove_self_loops(graph.edge_index).detach().cpu()
    a = scipy_csr_from_edges(edge, graph.num_nodes)
    a.data[:] = 1.0
    ...
    normalized = sp.diags(inv) @ a @ sp.diags(inv)
    try:
        emb = TruncatedSVD(n_components=2, random_state=random_state, n_iter=7).fit_transform(normalized)
    except Exception:
        rng = np.random.default_rng(random_state)
        emb = rng.standard_normal((graph.num_nodes, 2))
    ei_np = edge.numpy()
    delta = emb[ei_np[1]] - emb[ei_np[0]]
    angles = np.mod(np.arctan2(delta[:, 1], delta[:, 0]), 2.0 * np.pi)
    rel = np.floor(angles / (2.0 * np.pi / bins)).astype(np.int64)
    loops = torch.arange(graph.num_nodes, dtype=torch.long)
    full_edge = torch.cat([edge, torch.stack([loops, loops], dim=0)], dim=1)
    full_rel = torch.cat([torch.from_numpy(rel), torch.full((graph.num_nodes,), bins, dtype=torch.long)])
    return (full_edge, full_rel)
...
class GeomRelationLayer(nn.Module):

    def __init__(self, in_dim: int, out_dim: int, num_relations: int, merge: str='cat'):
        super().__init__()
        self.merge = merge
        self.num_relations = int(num_relations)
        self.transforms = nn.ModuleList([nn.Linear(in_dim, out_dim, bias=False) for _ in range(num_relations)])
        self.bias = nn.Parameter(torch.zeros(out_dim * num_relations if merge == 'cat' else out_dim))

    def forward(self, x: Tensor, edge_index: Tensor, relation: Tensor) -> Tensor:
        src_all, dst_all = edge_index
        n = x.size(0)
        deg = x.new_zeros(n)
        deg.index_add_(0, dst_all, torch.ones(dst_all.numel(), dtype=x.dtype, device=x.device))
        inv = deg.clamp_min(1.0).pow(-0.5)
        outs: List[Tensor] = []
        for rid, transform in enumerate(self.transforms):
            mask = relation == rid
            z = transform(x)
            if mask.any():
                src, dst = (src_all[mask], dst_all[mask])
                w = inv[src] * inv[dst]
                out = z.new_zeros((n, z.size(1)))
                out.index_add_(0, dst, z[src] * w.unsqueeze(-1))
            else:
                out = z.new_zeros((n, z.size(1)))
            outs.append(out)
        if self.merge == 'cat':
            ...
            result = torch.stack(outs, dim=0).mean(dim=0)
        else:
            raise ValueError(self.merge)
        return result + self.bias


class GeomGCNModel(nn.Module):

    def __init__(self, graph: GraphData, hidden_per_relation: int=16, dropout: float=0.5, bins: int=8, seed: int=0):
        super().__init__()
        edge, rel = build_geom_relations(graph, bins=bins, random_state=seed)
        self.register_buffer('relation_edge', edge)
        self.register_buffer('relation_id', rel)
        relations = bins + 1
        self.layer1 = GeomRelationLayer(graph.num_features, hidden_per_relation, relations, merge='cat')
        self.layer2 = GeomRelationLayer(hidden_per_relation * relations, graph.num_classes, relations, merge='mean')
        self.dropout = float(dropout)

    def forward(self, graph: GraphData) -> Tensor:
        h = row_normalize_features(graph.x)
        h = F.relu(self.layer1(h, self.relation_edge, self.relation_id))
        h = F.dropout(h, p=self.dropout, training=self.training)
        return self.layer2(h, self.relation_edge, self.relation_id)
...
def model_config(
    ...
    cfg = TrainConfig(hidden=16, layers=2, dropout=0.5, extras={'bins': 8})
    if quick:
...
def build_model(graph: GraphData, config: TrainConfig, seed: int) -> nn.Module:
    return GeomGCNModel(
        graph, config.hidden, config.dropout,
        int(config.extras['bins']), seed,
    )
