#!/usr/bin/env python3
"""Standalone, model-only MLP baseline runner.

Example: python mlp.py 0
Dataset indices: 0=Cora, 1=Citeseer, 2=Pubmed.
Only the MLP implementation and the shared data/training path needed by it are included.
"""
...
class MLPNet(nn.Module):
    ...
    def __init__(self, in_dim: int, hidden: int, classes: int, dropout: float=0.5, layers: int=2):
        super().__init__()
...
def model_config(
    ...
    patience = 100 if data_id in PLANETOID_NAMES else 200
    cfg = TrainConfig(patience=patience, hidden=64, layers=2, dropout=0.5)
    if quick:
...
def build_model(graph: GraphData, config: TrainConfig) -> nn.Module:
    return MLPNet(
        graph.num_features, config.hidden, graph.num_classes,
        config.dropout, config.layers,
    )
