#!/usr/bin/env python3
"""Standalone, model-only GPRGNN baseline runner.

Example: python gprgnn.py 0
Dataset indices: 0=Cora, 1=Citeseer, 2=Pubmed.
Only the GPRGNN implementation and the shared data/training path needed by it are included.
"""
...
class GPRGNNModel(nn.Module):
    ...
    def __init__(self, graph: GraphData, hidden: int=64, dropout: float=0.5, dprate: float=0.5, k: int=10, alpha: float=0.1):
        super().__init__()
        self.lin1 = nn.Linear(graph.num_features, hidden)
        self.lin2 = nn.Linear(hidden, graph.num_classes)
        self.prop = GPRPropagation(k=k, alpha=alpha, init='PPR')
    ...
    def forward(self, graph: GraphData) -> Tensor:
        ...
        x = F.dropout(graph.x, p=self.dropout, training=self.training)
        x = F.relu(self.lin1(x))
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = self.lin2(x)
        x = F.dropout(x, p=self.dprate, training=self.training)
        return self.prop(x, edge, weight)

    def optimizer_groups(self, config: 'TrainConfig') -> List[Dict[str, Any]]:
        return [{'params': self.lin1.parameters(), 'lr': config.lr, 'weight_decay': config.weight_decay}, {'params': self.lin2.parameters(), 'lr': config.lr, 'weight_decay': config.weight_decay}, {'params': self.prop.parameters(), 'lr': config.lr, 'weight_decay': 0.0}]
...
def model_config(
    ...
    cfg = TrainConfig(hidden=64, layers=10, dropout=0.5, extras={'dprate': 0.5, 'alpha': 0.1})
    if quick:
...
def build_model(graph: GraphData, config: TrainConfig) -> nn.Module:
    ...
    return GPRGNNModel(graph, config.hidden, config.dropout, float(extras['dprate']), config.layers, float(extras['alpha']))
